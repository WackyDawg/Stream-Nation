$(document).ready(function () {
    ...
    $(".item-0-2-225").on("click", function () {
      ...
      var $targetDiv = $(".channelList-0-2-237").find(
        "[data-id='" + dataId + "']"
      );
      if ($targetDiv.length) {
        ...
        if (
          lastTargetId === dataId &&
          Math.abs(currentScrollTop - lastScrollTop) < containerHeight
        ) {
    ...
    $(".channelList-0-2-237").scroll(function () {
      // Iterate over each button to check if it corresponds to a visible div
      $(".item-0-2-225").each(function () {
        var dataId = $(this).attr("data-id");
        var $targetDiv = $(".channelList-0-2-237").find(
          "[data-id='" + dataId + "']"
        );
        if ($targetDiv.length) {
          ...
          // Check if the target div is within the visible portion of the container
          if (targetOffset >= 0 && targetOffset <= containerHeight) {
            // Remove the classes from all buttons
            $(".item-0-2-225").removeClass("selected selected-l2-category-atc");
  
            // Add the classes to the corresponding button
            $(this).addClass("selected selected-l2-category-atc");
  
            // Update lastTargetId and lastScrollTop
            lastTargetId = dataId;
